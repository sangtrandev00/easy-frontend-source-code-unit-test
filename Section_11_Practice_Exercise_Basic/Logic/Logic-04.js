export function findMostFrequentNumber(numberList) {
  ...
  const newArrRepeatValue = Object.values(objStatistics);
  const newArrNumberValue = Object.keys(objStatistics);

  const maxRepetition = Math.max(...newArrRepeatValue);
  for (let i = 0; i < newArrRepeatValue.length; i++) {
    if (maxRepetition === newArrRepeatValue[i]) {
      return Number(newArrNumberValue[i]);
    }
  }
}
